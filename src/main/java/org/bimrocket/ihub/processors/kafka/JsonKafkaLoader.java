...
public class JsonKafkaLoader extends KafkaLoader
{
  private static final Logger log = LoggerFactory
      .getLogger(JsonKafkaLoader.class);

  @ConfigProperty(name = "objectType", description = "The object type to load")
  ...
  public boolean processObject(ProcessedObject procObject)
  {
    ...
    else
    {
      ...
      {
        log.debug(
            "record not a valid json, this should never happen incoming record::{}",
            recordLoaded);
        procObject.setOperation(ProcessedObject.IGNORE);
        return false;
      }
